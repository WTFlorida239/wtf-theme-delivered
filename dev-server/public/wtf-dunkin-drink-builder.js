...
window.WTFDunkinBuilder = (function() {
    ...
    async addToCart() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          let lineItem = null;
          try {
            lineItem = await response.json();
          } catch (jsonError) {
            console.warn('Unable to parse add-to-cart response', jsonError);
          }
          ...
          try {
            ...
            console.warn('Unable to fetch cart state', cartError);
          }

          document.dispatchEvent(new CustomEvent('wtf:cart:update', { detail: { cart, last_added: lineItem } }));
  ...
  return {
    init: function(target, config) {
      ...
      } else if (target instanceof Element || target instanceof DocumentFragment) {
        elements = [target];
